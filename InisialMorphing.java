...
*/
public class InisialMorphing extends Frame
{

  //Constructor
  public InisialMorphing()
  {
  ...
  public void paint(Graphics g)
  {
    ...
    //Control points for the letter Z.
    int[] xz = new int[6]; //The index 0 is not used.
    int[] yz = new int[6]; //The index 0 is not used.
    ...
    //Number of steps (frames) that the transform from C to Z should take.
    int step = 500;
    double stepDouble = step; //Number of steps as a Double-value to avoid
                                //repeated casting.
    for (int i=1; i<=step; i++)
    {
      ...
      for (int j=1; j<x.length; j++)
      {
        x[j] = (1-alpha)*xc[j] + alpha*xz[j];
        y[j] = (1-alpha)*yc[j] + alpha*yz[j];
      }
  ...
  */
  ...
  {
    InisialMorphing f = new InisialMorphing();
